...
import pt.isel.ls.webApi.dto.CourtDTO
import pt.isel.ls.webApi.dto.ResponseCourtDto
import pt.isel.ls.webApi.CourtWebApi
import pt.isel.ls.webServices.*
import kotlin.test.assertTrue

class CourtWebApiTests {
    private val courtWebApi = CourtWebApi(CourtServices)
    @Test
    fun `should create court successfully`() {
        val courtDto = CourtDTO(id = 1, name = "Court 1")
        ...
        val actualResponse = Json.decodeFromString<ResponseCourtDto>(responseBody)
        assertTrue(actualResponse.crid > 0)
    }
    ...
    fun `should return OK if club has courts`() {
        ...
        val response = CourtWebApi(CourtServices).appCourts(request)
