...
class ClubDataMemTest {
    ...
    fun `test createClub adds club correctly`() {
        ...
        val club = ClubDataMem.createClub(Name("Tennis Club"), user.token)
        assertNotNull(club)
    ...
    @Test
    fun `test createClub returns null for invalid token`() {
        val club = ClubDataMem.createClub(Name("Fake Club"), Token("invalid-token"))
        assertNull(club)
    }
    ...
    fun `test getClubById returns correct club`() {
        ...
        val createdClub = ClubDataMem.createClub(Name("Chess Club"), user.token)
