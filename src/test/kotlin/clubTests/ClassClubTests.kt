...
class ClassClubTest {
    ...
    fun `Club Valid`() {
        val club = Club(Id(1), Name("Padel Club"),
            Owner(User(Id(10), Name("Michael"), Email("michael@gmail.com"),Token(UUID.randomUUID().toString()) )))
    ...
    fun `Club with Invalid Owner ID`() {
        assertFailsWith<IllegalArgumentException> {
            Club(
                Id(6),
                Name("Padel Masters"),
                Owner(User(Id(-5), Name("Michael"), Email("michael@gmail.com"),Token(UUID.randomUUID().toString()) ))
            )
        }
    }
}